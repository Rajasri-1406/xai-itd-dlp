"""
File sharing model — MongoDB collections for files and approval requests
"""
...
def get_files_for_employee(email):
    ...
    1. Public files
    2. Private files where employee is in allowed_emails
    3. Files that have been approved-forwarded to this employee
    """
    from bson import ObjectId

    # Base query: public + explicitly allowed private
    ...
    # Also find files forwarded to this employee (approved forward requests)
    ...
    if forwarded_reqs:
        # Collect file_ids already in our list to avoid duplicates
        existing_ids = {str(f["_id"]) for f in files}
        fwd_file_ids = [r["file_id"] for r in forwarded_reqs if r["file_id"] not in existing_ids]

        if fwd_file_ids:
            for fid in set(fwd_file_ids):
                try:
                    fobj = shared_files_col.find_one({"_id": ObjectId(fid), "is_active": True})
                    if fobj and str(fobj["_id"]) not in existing_ids:
                        fobj["forwarded"] = True  # flag so UI can show "Forwarded" badge
                        files.append(fobj)
...
def create_approval_request(file_id, file_name, requested_by,
    ...
    # Always store file_id as plain string to avoid ObjectId vs string mismatch
    file_id = str(file_id)
